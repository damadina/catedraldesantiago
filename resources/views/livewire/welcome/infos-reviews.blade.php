<div class="mt-4">
    ...
    <div class="card">
        <div class="card-body">
            <p class="text-gray-800 text-xl">{{$info->reviews->count()}} comentarios</p>
            @foreach ($info->reviews->reverse() as $review)
                <div class="flex mb-4 text-gray-800">
                    ...
                    <div class="card flex-1">
                        <div class="card-body bg-gray-100">
                            <p class="p-2 font-semibold">{{$review->user->name}}</p>
                            <div class="flex">
                               <p class="flex-1 px-2">{{$review->comment}}</p>
                                @auth
                                    @if(Auth::user()->id == $review->user_id)
                                        <div class="pr-2 pb-2">
                                            <button wire:click="$emit('delete',{{$review->id}})" class="btn btn-danger text-xs">Eliminar mi comentario</button>
                                        </div>
                                    @endif
                                @endauth
                            </div>
